...
One useful approach is to simulate draws from the data holding covariates constant at quantities of interest, and then to show the results in a log-log plot similar to those presented in an earlier section. Figure 7 offers an example. Results for both the *Baseline* and the *Covariate* models are shown. For the latter, war deaths were simulated holding covariates constant at their values for World War II. For reference, the empirical CDF for World War II is shown in the plot as well.
